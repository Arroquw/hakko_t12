...
// uint8_t IRON_CFG::tipsActivated() {
//   return tips_activated;
// }
...
bool IRON_CFG::toggleTipActivation(uint8_t global_index) {
  ...
  if (indx >= 0) {            // tip calibration data found in EEPROM
    Serial.println("Loading tip data ...");
    Serial.println(tip_data.tip_name);
    if (loadTipData(&tip_data, indx)) {  // The tip calibration data is valid
      tip_data.mask ^= TIP_ACTIVE;       // Toggle tip activation flag
      init_tip_data = false;
    } else {
      Serial.println("INVALID TIP CALIBRATION DATA");
    }
  } else {
    indx = emptyTipSlot();
    if (indx < 0) {
      Serial.println("Failed loading tip data ...");
      Serial.println(tip_data.tip_name);
      return false;
    }  // Failed to save yet another tip calibration data
  }
  if (init_tip_data) {
    TIPS::name(tip_data.tip_name, global_index);
    uint32_t cd = def_tip[2] & 0x3FF;
    cd <<= 10;  // Pack tip calibration data in one 32-bit word: 10-bits per value
    cd |= def_tip[1] & 0x3FF;
    cd <<= 10;
    cd |= def_tip[0];
...
bool IRON_CFG::save(void) {
  if (memcmp(&Config, &previous_cfg, sizeof(struct cfg)) != 0) {
    Serial.print(F("update temp: "));
    Serial.print(Config.temp);
    Serial.print(F(", tip: "));
    Serial.print(Config.tip);
    Serial.print(F(", lo_temp: "));
    Serial.print(Config.low_temp);
    Serial.print(F(", lo_to: "));
    Serial.print(Config.low_to);
    Serial.print(F(", off: "));
    Serial.print(Config.off_timeout);
    Serial.print(F(", mask: "));
    Serial.println(Config.bit_mask, BIN);
    CONFIG::save();
    memcpy(&previous_cfg, &Config, sizeof(struct cfg));
  }
}
