...
public:
  ...
  void shortBeep(void) {
    if (on) tone(buzzer_pin, 3520, 160);
  }
  void lowBeep(void) {
    if (on) tone(buzzer_pin, 880, 160);
  }
...
void tipSCREEN::init(void) {
  ...
  Serial.print("tipsactivated: ");
  Serial.println(pCfg->tipsLoaded());
  pEnc->reset(old_tip, 0, pCfg->tipsLoaded() - 1, 1, 1, true);  // Select the tip by the rotary encoder
  forceRedraw();
  pD->clear();
  pD->msgSelectTip();
}
...
void tipSCREEN::rotaryValue(int16_t value) {
  ...
  if (value == old_tip)
    return;
  uint8_t max_val = pCfg->tipsLoaded() - 1;
  if (old_tip == 0 && value == max_val) {
    forward = false;
  } else if (old_tip == max_val && value == 0) {
    forward = true;
  } else {
    forward = value > old_tip;
  }
  update_screen = millis() + period;
  uint8_t index = pCfg->nextTip(old_tip, forward);
...
DSPL disp;
RENC encoder(R_MAIN_PIN, R_SECD_PIN, R_BUTN_PIN, ENC_INIT_POS);
