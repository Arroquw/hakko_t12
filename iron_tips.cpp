...
static const char tip_names[][tip_name_sz] PROGMEM = {
  { 'B', '\0', '\0', '\0', '\0' },
  { 'B', '1', '\0', '\0', '\0' },
  { 'B', '2', '\0', '\0', '\0' },
  { 'B', '3', '\0', '\0', '\0' },
...
bool TIPS::name(char tip_n[tip_name_sz], uint8_t index) {
  if (index < tip_number) {
    for (uint8_t c = 0; c < tip_name_sz; ++c) {
      tip_n[c] = pgm_read_byte(&tip_names[index][c]);
    }
    return true;
  } else {
    for (uint8_t c = 0; c < tip_name_sz; ++c)
...
int8_t TIPS::index(const char name[tip_name_sz]) {
  for (uint16_t i = 0; i < tip_number; ++i) {
    char tip_n[tip_name_sz];
    for (uint8_t c = 0; c < tip_name_sz; ++c)
      tip_n[c] = pgm_read_byte(&tip_names[i][c]);
    if (strncmp(name, tip_n, tip_name_sz) == 0) {
      return i;
    }
  }
  return -1;
