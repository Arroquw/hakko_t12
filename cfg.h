 ...
 */
typedef enum { CFG_CELSIUS = 1,
               CFG_BUZZER = 2,
               CFG_SWITCH = 4,
               CFG_THERM = 8 } CFG_BIT_MASK;
...
public:
  IRON_CFG(uint8_t max_custom_tips)
    : CONFIG(max_custom_tips) {}
  void init(void);
  bool isCelsius(void) {
  ...
  uint16_t lowTemp(void) {
    return Config.low_temp;
  }
  uint8_t lowTimeout(void) {
    return Config.low_to;
  }
